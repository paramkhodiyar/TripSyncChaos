      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="grid md:grid-cols-3 gap-8 items-start">
          <div className="md:col-span-2">
            ...
            <div className="mb-10">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold bg-blue-50 text-blue-500 px-2 py-0.5 rounded-full">{activeGroups.length}</span>
                </div>
                <Link
                  ...
                  {activeGroups.map((gm) => (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-slate-500 text-xs mt-1 truncate font-medium">{gm.group.description || "No description provided."}</p>
                      </div>
                      <div className="flex items-center justify-between mt-2 pt-3 border-t border-slate-50">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1">
                          <Users size={10} /> {gm.group._count.members}
                        </span>
                        {gm.group.isStarted && <span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-100">Started</span>}
                        <span className="text-[10px] font-bold text-blue-500 bg-blue-50 px-2 py-0.5 rounded-md uppercase tracking-widest">{gm.role}</span>
                      </div>
                    </Link>
                  ))}
                </div>
            ...
            {/* Separator */}
            {archivedGroups.length > 0 && <div className="h-px bg-slate-200 w-full my-8" />}
